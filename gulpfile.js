...
const config = require('./goodthree.config');
...
function js(done) {
  return src(
		...
		.pipe(terser())
		.pipe(dest(config.js.dist, { sourcemaps: '.' }));
};
...
function clean() {
    return del(config.themedir+'/dist/**', {force:true});
};
...
exports.default = series(parallel(css, js, img, fonts, icons), browser_sync, watchFiles);
exports.production = series(parallel(css, js, img, fonts, icons));
